...
def actualizar_rastreo(primer_id, rastreo_id, ids_esta_frame, frames_perdidos, frames_espera=10):
    ...
    if primer_id is None and ids_esta_frame:
        ...
        if rastreo_id not in ids_esta_frame:
            frames_perdidos += 1  # Incrementamos los frames perdidos cuando el ID rastreado no está presente
            if frames_perdidos < frames_espera:
                print(f"ID {rastreo_id} ausente durante {frames_perdidos} frames. Siguiendo esperando...")
                # Aún no hemos llegado a los frames de espera, seguimos esperando
            else:
                print(f"ID {rastreo_id} ausente durante {frames_perdidos} frames. Buscando nuevo objetivo...")
                # Si hemos esperado el tiempo suficiente, buscamos un nuevo objetivo
                if ids_esta_frame:
                    # Asignamos el nuevo ID disponible
                    nuevo_id = next(iter(ids_esta_frame))
                    print(f"Ahora rastreando a la nueva persona: ID {nuevo_id}")
                    rastreo_id = nuevo_id
                    frames_perdidos = 0  # Reiniciamos los frames perdidos porque cambiamos de objetivo
                    return primer_id, rastreo_id, True, frames_perdidos
        else:
            frames_perdidos = 0  # El ID rastreado sigue visible, reiniciamos el contador de frames perdidos
